...
class BiGGNN(nn.Module):
    def __init__(self, config):
        ...
        hidden_size = config.hidden_size  # 隐藏层的大小，超参数，小型网络一般在64， 128， 256
        self.hidden_size = hidden_size
        self.graph_direction = config.graph_direction  # 边的方向
        # self.graph_hops = config.graph_hops  # 图神经网络的 hops 数
        self.graph_hops = config.hops
        # self.word_dropout = config.word_dropout  # 丢弃词向量，一般在0~1之间
    ...
    def forward(self, node_feature, edge_vec, adj_in, adj_out, config):  # batch_size取值在16~128之间比较合理
        # node_feature是一个节点特征矩阵，其维度为(batch_size, num_nodes, dim)，
        # 其中batch_size表示批次数，num_nodes表示节点数量，dim表示每个节点的维度。
        # edge_vec是一个边向量矩阵，其维度为(batch_size, num_edges, dim)。
        # adj是一个邻接矩阵，其维度为(batch_size,num_nodes,num_nodes)。
        ...
        # output = self.classifier(node_state[0])
        return node_state

    # biggnn的图更新，输入h(t-1)节点特征矩阵、边向量和邻接矩阵，输出h(t)节点特征矩阵
    def static_graph_update(self, node_feature, edge_vec, adj_in, adj_out):  # 这里输入的adj应该是两个方向邻接矩阵拼起来的
        ''' Static graph update '''
        node2edge = adj_in
        edge2node = adj_out
        # node2edge, edge2node = adj
        # Shape: (batch_size, num_edges, num_nodes)
        # node2edge = to_cuda(torch.stack([torch.Tensor(x.A) for x in node2edge], dim=0), self.device)  # 向后邻接矩阵
        # Shape: (batch_size, num_nodes, num_edges)
        # edge2node = to_cuda(torch.stack([torch.Tensor(x.A) for x in edge2node], dim=0), self.device)  # 向前邻接矩阵
...
class GraphMessagePassing(nn.Module):
    def __init__(self, config):
        ...
        if config.message_function == 'edge_mm':  # 初始化边权矩阵
        ...
        elif config.message_function == 'edge_network':  # 初始化边网络,应该用这个吧
        ...
        elif config.message_function == 'edge_pair':  # 初始化线性层
            self.linear_edge = nn.Linear(config['edge_embed_dim'], hidden_size, bias=False)
            self.mp_func = self.msg_pass
        elif config.message_function == 'no_edge':  # 不需要初始化任何参数，这个表示边没有特征
            self.mp_func = self.msg_pass
        else:
